	...
	if(isset( $current_screen->id ) && $current_screen->id == 'toplevel_page_zoho_flow'){
		if ( ! get_option( 'zoho_flow_admin_footer_text_rated' ) ) {
			$footer_text = sprintf(
				...
				esc_html__( 'If you love using %1$s, please leave a %2$s rating. A huge thanks in advance!', 'zoho-flow' ),
...
function zoho_flow_review_banner(){
	$last_review_notice_date = get_option('last_review_notice_date');
	$current_date = date("Y-m-d");
	//Review banner will be shown once in 3 months
		if ( (!empty($last_review_notice_date)) && (date_diff(new DateTime($current_date) ,new DateTime($last_review_notice_date),false )->format('%m') >= 3)){
?>
		<div id= "review_notice" style="padding: 10px" class="notice notice-info is-dismissible">
			<?php
			echo sprintf(
				esc_html__('Happy with %1$s plugin? Please take a moment to leave us a %2$s'),sprintf('<strong>%s</strong>',esc_html__('Zoho Flow')), '<a href="https://wordpress.org/support/plugin/zoho-flow/reviews?rate=5#new-post" target="_blank" class="zoho-flow-rating-link" >review </a>'
			);
			?>
		</div>
<?php
			update_option( 'last_review_notice_date', $current_date );
			}
			else if(empty($last_review_notice_date)){
				add_option( 'last_review_notice_date', $current_date, '', 'yes' );
			}
}
...
<div class="plugin-gallery-wrapper">
	<div class="plugin-gallery">
		...
		<p><?php echo esc_html__( 'Connect your WordPress plugins to Zoho Flow to create integrations. Click to access their API keys and other connection details.', 'zoho-flow' )?></p>
...
?>
	...
	<div class="plugin-gallery-right-side">
		...
		<div class="flow-five-years-section">
			<h2><?php echo esc_html__('Our journey so far') ?></h2>
			<div class="flow-five-years">
				<a href="https://www.zoho.com/flow/5-years.html" style="text-decoration:none !important; outline: 0 !important;" target="_blank">
					<img src="<?php echo plugins_url('../assets/images/five-year-blog-banner.jpg', __FILE__); ?>">
				</a>
			</div>
		</div>
	</div>
...
<div id="api_key_details" style="display:none;">
	...
	<div id="api-key-div" style="display:none; font-size: 13px;">
		...
		<p style="margin:0px;"><?php echo esc_html__('Copy the API key and keep it safe. The API key can\'t be copied or retrieved again.', 'zoho-flow') ?></p>
		<span id="api-key-span"></span>
		<a id="copy-api-key" style="padding: 10px 0px;" class="dashicons dashicons-admin-page"></a>
		<span id='copy-api-key-sucess'>Copied!</span>
		<h3><?php echo esc_html('Base URL', 'zoho-flow') ?></h3>
		<span id="site-url-span"><?php echo esc_html(site_url(),'zoho-flow')?></span>
		<a id="copy-site-url" style="padding: 10px 0px;" class="dashicons dashicons-admin-page"></a>
		<span id='copy-site-url-sucess'>Copied!</span>
